...
/**
 * Implementation of the payment network to pay in StarNotary based on a reference provided to a proxy contract.
 ...
 * Every StarNotary ethereum transaction that reaches these addresses through the proxy contract and has the correct reference will be interpreted as a payment or a refund.
 ...
 */
const starNotaryProxyContract: ExtensionTypes.PnReferenceBased.IReferenceBased = {
...
/**
 * Creates the extensionsData to create the extension StarNotary proxy contract payment detection
 *
...
function createCreationAction(
  ...
  return ReferenceBased.createCreationAction(
    ExtensionTypes.ID.PAYMENT_NETWORK_STARNOTARY_PROXY_CONTRACT,
    creationParameters,
    CURRENT_VERSION,
  );
}
...
function applyActionToExtension(
  ...
  if (
    requestState.currency.type !== RequestLogicTypes.CURRENCY.STARNOTARY ||
    (requestState.currency.network && !supportedNetworks.includes(requestState.currency.network))
  ) {
    throw Error(
      `This extension can be used only on StarNotary requests and on supported networks ${supportedNetworks.join(
        ', ',
      )}`,
    );
  }
...
export default starNotaryProxyContract;